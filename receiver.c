...
#define TARGET_IP "192.168.202.211"
uint16_t lcdPixels[480][320];

char charRed[3], charGreen[3], charBlue[3];
...
{ 
  ...
  // sendto(sockfd, "1", 1, 0, (struct sockaddr*)& addrDest, sizeof(addrDest));
  // printf("SENT\n");
  // printf("%d\n", sockfd);
 
  recvfrom(sockfd, buffer_tx, sizeof(buffer_tx), 0, (struct sockaddr*)&from, &fromlen);
  printf("received");
  printf(buffer_tx);
  memset(charRed, 0, 3);
  memcpy(charRed, buffer_tx, 3);
  color.r = atoi(charRed);

  memset(charGreen, 0, 3);
  memcpy(charGreen, buffer_tx + 3, 3);
  color.g = atoi(charGreen);

  memset(charBlue, 0, 3);
  memcpy(charBlue, buffer_tx + 6, 3);
  color.b = atoi(charBlue);

  *(volatile uint32_t*)(mem_base + SPILED_REG_LED_RGB1_o) = rgbtohex(&color);
  *(volatile uint32_t*)(mem_base + SPILED_REG_LED_RGB2_o) = 0x00FFFF;
